...
export async function updateTeamBySlug(
  ...
  const result = await query(
    "update teams set name = $1, description = $2 where slug = $3 returning *",
...
export async function deleteTeamBySlug(slug: string): Promise<Team | null> {
  const result = await query("delete from teams where slug = $1 returning *", [
    slug,
  ]);
  if (!result || !result.rows || result.rows.length !== 1) {
    return null;
...
export async function getGamesByTeam(
  ...
  const result = await query(
    "select * from games where home = $1 or away = $1 order by date",
    [slug]
  );
  if (!result || !result.rows) {
    return [];
...
export async function updateGameById(
  ...
  const result = await query(
    "update games set date = $1, home = $2, away = $3, home_score = $4, away_score = $5 where id = $6 returning *",
...
export async function deleteGameById(id: number): Promise<Game | null> {
  const result = await query("delete from games where id = $1 returning *", [
    id,
  ]);
  if (!result || !result.rows || result.rows.length !== 1) {
    return null;
